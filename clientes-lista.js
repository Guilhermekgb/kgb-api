import { firebaseClientes } from "./api/firebase-clientes.js";
...
async function carregarClientes() {
 ...
 // 1) Busca na NUVEM (FIREBASE)
let clientesRemotos = [];
try {
  clientesRemotos = await firebaseClientes.list();
} catch (e) {
  console.warn('[clientes] erro ao buscar no Firebase:', e);
}

  // clientes locais (varias chaves conhecidas ou varredura do localStorage)
  let clientesLocais = [];
  try {
    clientesLocais = lerClientesLocalPorChaves() || scanLocalStoragePorClientes() || [];
  } catch (e) {
    clientesLocais = [];
  }
