...
function getUsuarioAtual(){
  try {
    const u1 = JSON.parse(localStorage.getItem("usuarioLogado") || "null");
    if (u1 && (u1.email || u1.nome || u1.id)) return u1;
  } catch {}
  try {
    const u2 = JSON.parse(localStorage.getItem("auth:user") || "null");
    if (u2 && (u2.email || u2.nome || u2.id)) return u2;
  } catch {}
