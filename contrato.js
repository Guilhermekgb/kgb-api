...
(() => {
  ...
  async function aplicarModelo(){
    ...
    // Nome do usuário vindo da sessão (cookie + /auth/me).
    async function __getNomeUsuarioAtual(){
      try {
        ...
        return (u && u.nome) ? u.nome : "Usuário";
      } catch { return "Usuário"; }
    }

    let usuarioAtual = "Usuário";
    try { usuarioAtual = (window.__KGB_USER_CACHE && window.__KGB_USER_CACHE.nome) ? window.__KGB_USER_CACHE.nome : usuarioAtual; } catch {}
    // aplica async quando necessário (aplicarModelo é acionado por usuário)
    // garante que usuarioAtual seja obtido via sessão quando possível
    try { usuarioAtual = await __getNomeUsuarioAtual(); } catch(e){}
