...
function renderSaudacaoUsuario(){
  ...
  // tenta extrair o nome de diferentes formatos
  const primeiroValido = cand.find(x => x && typeof x === 'object') || {};
  let nome =
      primeiroValido.nome ||
      primeiroValido.name ||
...
(function wireFormLancRapido(){
  ...
  // Aviso para ajudar a identificar quando o form não existe
  if (!form) {
    console.warn('[dashboard] formLancRapido NÃO encontrado. Verifique se o modal tem <form id="formLancRapido">.');
    return;
  }
...
function renderGraficoConversaoCarol(){
  try {
    const leadsLS = JSON.parse(localStorage.getItem('leads') || '[]');
