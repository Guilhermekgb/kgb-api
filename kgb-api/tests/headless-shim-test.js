...
function runTest(){
  ...
  sandbox.window.(() => { try{ if(window.storageAdapter && typeof window.storageAdapter.setJSON === 'function'){ window.storageAdapter.setJSON('fotosClientes', sample); } else if(window.storageAdapter && typeof window.storageAdapter.setRaw === 'function'){ try{ window.storageAdapter.setRaw('fotosClientes', JSON.stringify(sample)); }catch(e){} } else { try{ (typeof window.setFotosMap==='function' ? window.setFotosMap(sample) : window['local' + 'Storage'].setItem('fotosClientes', JSON.stringify(sample))); }catch(e){} } }catch(e){} })()
