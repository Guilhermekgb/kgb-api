...
(async () => {
  const base = getBase();
  const cookie = await loginFallback();
  if (!cookie) {
  ...
  const r1 = await httpGet(`${base}/portal/eventos/${eid}/financeiro`, cookie);
  console.log(`OK /portal/eventos/${eid}/financeiro ->`, r1.status);
  if (r1.status !== 200) process.exit(1);

  const r2 = await httpGet(`${base}/portal/eventos/${eid}/parcelas`, cookie);
  console.log(`OK /portal/eventos/${eid}/parcelas ->`, r2.status);
  if (r2.status !== 200) process.exit(1);

  const r3 = await httpGet(`${base}/portal/eventos/${eid}/timeline`, cookie);
  console.log(`OK /portal/eventos/${eid}/timeline ->`, r3.status);
