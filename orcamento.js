...
async function salvarLeadFunil(nextAction) {
  ...
  // ===== persistência segura =====
  try { saveLead?.(novoLead); } catch(e){ console.warn("saveLead falhou; aplicando fallback", e); }
  ...
  try {
    const k = "propostasIndex";
    const idx = JSON.parse(localStorage.getItem(k) || "[]") || [];
    const exists = idx.some(p => String(p.id) === String(novoLead.id));
    const item = {
      id: String(novoLead.id),
      cliente: novoLead.nome || "",
      dataISO: novoLead.dataEventoISO || "",
      local: novoLead.local || "",
      convidados: Number(novoLead.qtd||0)||0,
      status: novoLead.status || "Novo Lead"
    };
    if (!exists) idx.unshift(item); else {
  ...
  if ((usuarioLogado?.nome || usuarioLogado?.email) === novoLead.responsavel) {
    try { mostrarToast?.("Você é o responsável por este lead."); } catch {}
  }
...
function lerLeads(){  try{ return JSON.parse(localStorage.getItem('leads')  || '[]'); }catch{ return []; } }
