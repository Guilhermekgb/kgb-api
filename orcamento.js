  ...
  if (String(nextAction || '').toLowerCase() === 'proposta') {
    try {
      ...
      if (salvoOrc && salvoOrc.id) {
        ...
        // Compatibilidade: atualiza índice legada `propostasIndex` no localStorage
        try {
          const existingRaw = localStorage.getItem('propostasIndex') || '[]';
          let existing = [];
          try { existing = JSON.parse(existingRaw) || []; } catch { existing = []; }

          // monta entrada leve de proposta para aparecer nas listagens antigas
          const propEntry = {
            id: String(salvoOrc.id),
            leadId: String(novoLead.id),
            nome: novoLead.nome || '',
            dataEvento: novoLead.dataEvento || novoLead.dataEventoISO || '',
            local: novoLead.local || '',
            qtd: Number(novoLead.qtd || 0) || 0,
            status: 'enviado',
            criadoEm: new Date().toISOString(),
            visualizacoes: 0
          };

          // evita duplicados (mesmo id)
          existing = existing.filter(p => String(p.id) !== String(propEntry.id));
          existing.unshift(propEntry);
          // limite para evitar estourar quota
          if (existing.length > 600) existing = existing.slice(0, 600);
          localStorage.setItem('propostasIndex', JSON.stringify(existing));
          try { sessionStorage.setItem('propostasIndex', JSON.stringify(existing)); } catch {}
          try {
            // notifica outras partes da aplicação que as propostas mudaram
            try { window.dispatchEvent && window.dispatchEvent(new CustomEvent('propostas:changed', { detail: propEntry })); } catch(_){}
            // broadcast para outras abas/janelas (lista também escuta o channel 'mrubuffet')
            try { if (window.BroadcastChannel) new BroadcastChannel('mrubuffet').postMessage({ type: 'propostas:changed', id: propEntry.id }); } catch(_){}
            // pulso no localStorage para tentar acordar listeners que usam storage event
            try { localStorage.setItem('__kgb_propostas_changed', String(Date.now())); localStorage.removeItem('__kgb_propostas_changed'); } catch(_){}
          } catch(e){}
        } catch (e) { console.warn('[ORÇAMENTO] falha ao atualizar propostasIndex local:', e); }
      }
    } catch (e) {
      console.warn("[ORÇAMENTO] Erro ao salvar orçamento na API:", e);
    }

    // Se a API não retornou um orçamento (por falha de rede ou ausência de API),
    // garantimos compatibilidade escrevendo um índice local `propostasIndex`
    // para que páginas legadas e testes headless encontrem a proposta imediatamente.
    try {
      const existingRaw = localStorage.getItem('propostasIndex') || '[]';
      let existing = [];
      try { existing = JSON.parse(existingRaw) || []; } catch { existing = []; }

      const fallbackId = `prop_${novoLead.id}`;
      // monta entrada leve de proposta local
      const propEntryLocal = {
        id: fallbackId,
        leadId: String(novoLead.id),
        nome: novoLead.nome || '',
        dataEvento: novoLead.dataEvento || novoLead.dataEventoISO || '',
      ...
      // remove possíveis duplicados (id ou leadId)
      existing = existing.filter(p => String(p.id) !== String(propEntryLocal.id) && String(p.leadId) !== String(propEntryLocal.leadId));
      existing.unshift(propEntryLocal);
      if (existing.length > 600) existing = existing.slice(0, 600);
      localStorage.setItem('propostasIndex', JSON.stringify(existing));
      try { sessionStorage.setItem('propostasIndex', JSON.stringify(existing)); } catch {}
      try {
        try { window.dispatchEvent && window.dispatchEvent(new CustomEvent('propostas:changed', { detail: propEntryLocal })); } catch(_){}
        try { if (window.BroadcastChannel) new BroadcastChannel('mrubuffet').postMessage({ type: 'propostas:changed', id: propEntryLocal.id }); } catch(_){}
        try { localStorage.setItem('__kgb_propostas_changed', String(Date.now())); localStorage.removeItem('__kgb_propostas_changed'); } catch(_){}
      } catch(e){}
    } catch (e) { console.warn('[ORÇAMENTO] falha ao gravar propostasIndex (fallback):', e); }
  }
